...
バージョンの正本は`pubspec.yaml`の`version: 1.4.2+15`です。表示用バージョンは`1.4.2`、`+`以降はビルド番号です。Windowsの実行ファイルとインストーラー、macOSアプリとDMG、更新情報はビルド時にこの値を使用します。変更する場所は`pubspec.yaml`の1か所です。
...
Releaseフォルダは、`TNote.exe`、DLL、`data`を含むフォルダ全体を保持すればポータブル構成にできます。配布物には`TNote-Windows-Portable-1.4.2.zip`も用意します。別PCではフォルダ全体を展開し、必要なら同梱のWindows実行環境を先に導入します。正式な配布・関連付け・更新にはバージョン付きのインストーラーを使ってください。
...
- `TNote-Setup-1.4.2.exe`
- `TNote-Windows-Portable-1.4.2.zip`
- `TNote-1.4.2.dmg`
- `release-info.json`
